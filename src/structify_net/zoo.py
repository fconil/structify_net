...
def sort_distances(nodes, dimensions=1, distance="euclidean"):
    ...
    return stn.Rank_model(g, rank_function, node_order_function=node_order)
...
def sort_overlap_communities(nodes, blocks=None):
    ...
    if isinstance(blocks, int):
        ...
        block2 = _flatten(
            [[i + largest_id + 1] * block_size for i in range(nb_blocks_by_level)]
        )[:n]
...
def sort_largest_disconnected_cliques(nodes, m):
    ...
    c_size = math.ceil(
        (2 * m) / n + 1
    )  # Because we know the average degree, so the correst size for a clique
...
def sort_fractal_leaves(nodes, d=2):
    ...
    binary_tree = nx.relabel_nodes(
        binary_tree, {n: "temp_" + str(i) for i, n in enumerate(leaves_ids)}
    )
...
def sort_fractal_hierarchical(nodes, d=3):
    ...
    def child_of(u, v):
        return all_distances["temp_" + str(u)]["temp_" + str(v)] == abs(
            depths["temp_" + str(u)] - depths["temp_" + str(v)]
        )

    def child_score(u, v):
        return min(height_from_bottom(u), height_from_bottom(v)) + min(
            depths["temp_" + str(u)], depths["temp_" + str(v)]
        )
    ...
    def family_score(u, v):
        if depths["temp_" + str(u)] == depths["temp_" + str(v)]:  # same height
            return (
                all_distances["temp_" + str(u)]["temp_" + str(v)] - 2
            ) + height_from_bottom(u)
        else:
...
all_models_no_param = {
    "ER": sort_ER,
    "spatial": sort_distances,
    "spatialWS": sort_spatial_WS,
    "blocks_assortative": sort_blocks_assortative,
    "overlapping_communities": sort_overlap_communities,
    "nestedness": sort_nestedness,
    "maximal_stars": sort_stars,
    "core_distance": sort_core_distance,
    "fractal_leaves": sort_fractal_leaves,
    "fractal_root": sort_fractal_root,
    "fractal_hierarchy": sort_fractal_hierarchical,
    "fractal_star": sort_fractal_star,
    "perlin_noise": sort_perlin_noise,
}
