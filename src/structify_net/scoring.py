...
def is_degree_heterogeneous(graph):
    ...
    reference_threshold = _gini_coefficient(
        np.random.poisson(avg_degree, len(graph.nodes))
    )
    power_law_threshold = _gini_coefficient(
        nx.utils.powerlaw_sequence(len(graph.nodes), 3)
    )
    # print("-",degree_heterogeneity(graph),reference_threshold,power_law_threshold)
    return degree_heterogeneity(graph) > (
        (reference_threshold + power_law_threshold) / 3
    )
...
def degree_assortativity(
    graph, normalized=False, positive_only=True, disassortativity=False
):
...
def boundaries(graph, normalized=True):
    ...
    for c in coms:
        ...
        average_boundary_log = (density_intern - graph_density) - (
            density_extern - graph_density
        )  # /((1-graph_density)-(0-graph_density))
...
def scores_for_generators(
    generators, scores=None, runs=1, details=False, latex_names=True
):
...
def scores_for_rank_models(
    rank_models, m, scores=None, epsilons=0, runs=1, details=False, latex_names=True
):
    ...
    for eps in pbar:
        ...
        df_alpha = scores_for_generators(
            all_generators,
            scores=scores,
            runs=runs,
            details=details,
...
def scores_for_rank_functions(
    rank_functions, n, m, scores=None, epsilons=0, runs=1, latex_names=True
):
...
def compare_graphs(df_reference, df_graphs, best_by_name=False, score_difference=False):
    ...
    for i, row in df_graphs.iterrows():
        score_differences.append(
            [1 - abs(row[score] - scores_reference[score]) for score in all_scores]
        )

    df_diff_scores = pd.DataFrame.from_records(
        score_differences, columns=list(all_scores)
    )
    ...
    if best_by_name:
        df_to_return = (
            df_to_return.sort_values("distance").groupby("name").first().reset_index()
        )
...
score_names = {
    "transitivity": r"$CC(G)$",
    "average_clustering": r"$\overline{CC(u)}$",
    "coreness": "Core",
    "average_shortest_path_length": r"$\overline{d}$",
    "robustness": "Rob",
    "giant_component_ratio": "I",
    "modularity": r"$Q$",
    "boundaries": r"$Q_{bound}$",
    "degree_heterogeneity": r"$\sigma(k)$",
    "degree_assortativity": r"$$-(k \propto k)$$",
    "hierarchy": r"$${k \propto CC}$$",
    "nb_nodes": r"$n$",
    "nb_edges": r"$m$",
    "epsilon": r"$\epsilon$",
}
