...
class Graph_generator:
    ...
    def ER(n, p):
        ...
        Returns:
            nx.Graph: a graph
        """
        return transform._proba2graph(
            list(itertools.combinations(range(n), 2)),
            [p] * scipy.special.comb(n, 2, exact=True),
        )
...
class Rank_model:
    ...
    def _from_sorted_pairs(self, sortedPairs, node_properties=None, node_order=None):
        ...
        if self.node_order is None:
            self.node_order = list(
                set([u for u, v in self.sortedPairs] + [v for u, v in self.sortedPairs])
            )

    def __init__(
        self, nodes, sort_function, sort_descendent=False, node_order_function=None
    ):
        ...
        if node_order_function is None:
        ...
        sorted_pairs = {
            (u, v): sort_function(u, v, nodes) + random.random() / 1000
            for u, v in sorted_pairs
        }
        sorted_pairs = sorted(
            sorted_pairs.items(), key=lambda e: e[1], reverse=sort_descendent
        )
    ...
    def scores(
        self, m, scores=None, epsilons=0, runs=1, details=False, latex_names=True
    ):
        ...
        Returns:
            _type_: _description_
        """
        return scoring.scores_for_rank_models(
            self,
            m,
            scores=scores,
            epsilons=epsilons,
