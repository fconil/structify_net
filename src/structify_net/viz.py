...
def spider_plot(df, categories=None, reference=None):
    ...
    if "epsilon" in df.columns:
        df.rename(columns={"epsilon": r"$\epsilon$"}, inplace=True)
    if r"$\epsilon$" in df.columns:
        df["name"] = (
            df["name"] + r"($\epsilon$=" + df[r"$\epsilon$"].round(4).astype(str) + ")"
        )
        df.drop(columns=[r"$\epsilon$"], inplace=True)
    ...
    if len(df) > 3:
        ...
        nb_cols = 3
        fig_size = (
            each_spider[0] * nb_cols / my_dpi,
            each_spider[1] * nb_rows / my_dpi,
        )
...
def _make_spider(
    values, categories, title="", color="green", fill=True, ax=None, reference=None
):
    # number of variable
